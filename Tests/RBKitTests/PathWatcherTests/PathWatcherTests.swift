...
struct PathWatcherTests {
  ...
  func `pathMonitor, with existing path, should forward file system events`() async throws {
    ...
    try await withDependencies {
      $0.fileDescriptorClient.open = { _, mode, options in
        openCalled = true
        capturedMode = mode
  ...
  @Test
  func `pathMonitor, with non-existing path, should throw`() async {
    nonisolated(unsafe) var openCalled = false
  ...
  func `events, with successful stream creation, should yield events`() async throws {
    ...
    try await withDependencies {
      $0.fsEventStreamClient.create = {
        _, callback, context, paths, sinceWhen, latency, flags in
  ...
  @Test
  func `events, with stream creation failure, should throw streamCreationFailed`() async {
    await withDependencies {
  ...
  @Test
  func `events, with stream start failure, should throw streamStartFailed`() async {
    await withDependencies {
